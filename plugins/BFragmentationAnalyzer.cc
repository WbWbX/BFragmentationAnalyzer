  ...
  genJetsToken_(consumes<std::vector<reco::GenJet> >(edm::InputTag("particleLevel:jets")))
{
  ...
  for (std::string name: { "inc", "B" }) {
      histos_["xb_lead_"+name] = fs->make<TH1F>(("xb_lead_"+name).c_str(), (name+";x_{b}=p_{T}(B)/p_{T}(jet); Jets").c_str(), 123, xb_binning);
      histos_["xb_pt_lead_"+name] = fs->make<TH2F>(("xb_pt_lead_"+name).c_str(), (name+";x_{b}=p_{T}(B)/p_{T}(jet);p_{T}(jet);Jets").c_str(), 123, xb_binning, 8, pt_binning);
      histos_["xb_subLead_"+name] = fs->make<TH1F>(("xb_subLead_"+name).c_str(), (name+";x_{b}=p_{T}(B)/p_{T}(jet); Jets").c_str(), 123, xb_binning);
      histos_["xb_pt_subLead_"+name] = fs->make<TH2F>(("xb_pt_subLead_"+name).c_str(), (name+";x_{b}=p_{T}(B)/p_{T}(jet);p_{T}(jet);Jets").c_str(), 123, xb_binning, 8, pt_binning);
  }
